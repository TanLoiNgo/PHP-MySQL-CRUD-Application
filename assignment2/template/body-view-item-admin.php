...
$itemListCategory = getItemByCategory($db_con,$_GET['cat-id']);
...
<div id="item">
    <div id="sideCategory">
        ...
        <div>
            <?php foreach ($categoryList as $key => $value) { ?>
                ...
                <a href="./view-item-admin.php?cat-id=<?php echo $value["cat_id"]; ?>"
                   id="<?php echo $value["cat_id"]; ?>"> <?php echo $value["name"]; ?> </a>
    ...
    <div>
        <div class="flex-space-bwt">
            <h1>Items</h1>
            <a class="black-btn" href="./add-item.php">Add New Item</a>
        </div>

        <div>
            <?php
            if (isset($itemList)) {
                for ($i = $start; $i < ($limit * $current_page); $i++) {
                    if (!empty($itemList[$i])) {
                        if (isset($_GET['cat-id'])) {
                            if ($_GET['cat-id'] == 'all') {
                                ...
                                <div class="itemList">
                                    ...
                                    <div class="item-text">
                                        <h2><a href="./item-view-detail-admin.php?id=<?php echo $itemList[$i]['id'] ?>"><?php echo $itemList[$i]['title'] ?></a></h2>
                                        <p><?php echo $itemList[$i]['description'] ?></p>
                                        ...
                                        <div>
                                            <a class="black-btn"
                                               href="./modify-item.php?item-id=<?php echo $itemList[$i]['id'] ?>">Modify</a>
                                            <a class="black-btn"

                                               onclick="myFunction(<?php echo $itemList[$i]['id'] ?>)">Delete</a>
                                        </div>
                                        <div class="pictures">
                                            <?php
        ...
        <div>
            <?php
            if (isset($itemListCategory)) {
                for ($i = $start; $i < ($limit * $current_page); $i++) {
                    if (!empty($itemListCategory)) {
                        if ($i < sizeof($itemListCategory)) {
                            if (isset($_GET['cat-id'])) {
                                ?>
                                <div class="itemList">
                                    <div>
                                    ...
                                    <div class="item-text">
                                    <h2><a href="./item-view-detail-admin.php?id=<?php echo $itemListCategory[$i]['id'] ?>"><?php echo $itemListCategory[$i]['title'] ?></a></h2>
                                        <p><?php echo $itemListCategory[$i]['description'] ?></p>
                                        ...
                                        <div>
                                            <a class="black-btn"
                                               href="./modify-item.php?item-id=<?php echo $itemListCategory[$i]['id'] ?>">Modify</a>
                                            <a class="black-btn"
                                               onclick="myFunction(<?php echo $itemListCategory[$i]['cat_id'] ?>)">Delete</a>
                                        </div>
                                        <div class="pictures">
                                            <?php
        ...
        <div class="pagination">
            ...
            if ($current_page > 1 && $total_page > 1) {
                echo '<a href="./view-item-admin.php?cat-id=' . $pageCat . '&page=' . ($current_page - 1) . '">&laquo;</a> ';
            }
            for ($i = 1; $i <= $total_page; $i++) {
                ...
                if ($i == $current_page) {
                    ...
                    echo '<a href="./view-item-admin.php?cat-id=' . $pageCat . '&page=' . $i . '">' . $i . '</a> ';
                }
            }

            // if current_page < $total_page and total_page > 1, Next will be showed
            if ($current_page < $total_page && $total_page > 1) {
                echo '<a href="./view-item-admin.php?cat-id=' . $_GET['cat-id'] . '&page=' . ($current_page + 1) . '">&raquo;</a> ';
            }
            ?>
        </div>
    </div>
